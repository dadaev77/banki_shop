        ...
        if ($request->hasFile('images')) {
            ...
            foreach ($files as $file) {
                ...
                if (!file_exists($thumbnailPath)) {
                    if (!mkdir($thumbnailPath, 0755, true) && !is_dir($thumbnailPath)) {
                        throw new \RuntimeException(sprintf('Directory "%s" was not created', $thumbnailPath));
                    }
                }
    ...
    public function downloadZip($filename)
    {
        ...
        if (!is_dir($tempPath)) {
            if (!mkdir($tempPath, 0755, true) && !is_dir($tempPath)) {
                throw new \RuntimeException(sprintf('Directory "%s" was not created', $tempPath));
            }
        }
